...
#ifndef _SENSOR_SOIL_TEMPERATURE_H_
#define _SENSOR_SOIL_TEMPERATURE_H_

#include "Sensor.h"

class SensorSoilTemperature : public Sensor
{
public:
    SensorSoilTemperature(std::string zoneNumber, std::string serviceType,
            std::string equipmentIdentification, std::string equipmentId, DataStorageUnit* dataStorageUnit);
    SensorSoilTemperature(std::string equipmentId, DataStorageUnit* dataStorageUnit);
    ~SensorSoilTemperature() = default;
...
#endif // _SENSOR_SOIL_TEMPERATURE_H_
