...
SensorSoilMoisture::SensorSoilMoisture(std::string zoneNumber, std::string serviceType,
...
: Sensor(zoneNumber, serviceType, equipmentIdentification, equipmentId, dataStorageUnit)
{
    ...
    std::string sql = "CREATE TABLE IF NOT EXISTS Sensor009 (" \
                      "TIME TEXT NOT NULL," \
                      "DATA TEXT NOT NULL," \
                      "AAA TEXT NOT NULL," \
                      "AAAI TEXT NOT NULL," \
    ...
    {
        std::cout << "Table Sensor009 created successfully" << std::endl;
    }
}

SensorSoilMoisture::SensorSoilMoisture(std::string equipmentId, DataStorageUnit* dataStorageUnit)
...
void SensorSoilMoisture::StoreData(std::vector<std::string> data, std::string originalData)
{
    ...
    std::string sql = "INSERT INTO Sensor009 (TIME, DATA, AAA, AAAI, AAA5i, AAAl5i, FILTER) VALUES ('" + time + "', '" + originalData + "', '" + dataAAA + "', '" + dataAAAI + "', '" + dataAAA5i + "', '" + dataAAAl5i + "', " + filter + ")";
...
std::vector<std::string> SensorSoilMoisture::CalculateData(std::string startTime, std::string endTime)
{
    // Get data of current hour from database table Sensor009
    auto database = GetDataStorageUnit()->GetDatabase();
    ...
    std::string sql = "SELECT * FROM Sensor009 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "' ORDER BY TIME DESC LIMIT 1";
    sqlite3_stmt* stmt;
    ...
    // Get max AAA value
    sql = "SELECT MAX(AAA) FROM Sensor009 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "'";
    ...
    sql = "SELECT TIME FROM Sensor009 WHERE AAA = '" + AAAa + "'";
    rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
        std::cout << "SQL error: " << sqlite3_errmsg(database) << std::endl;
    ...
    // Get min AAAc value
    sql = "SELECT MIN(AAA) FROM Sensor009 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "'";
    rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
        std::cout << "SQL error: " << sqlite3_errmsg(database) << std::endl;
    ...
    sql = "SELECT TIME FROM Sensor009 WHERE AAA = '" + AAAc + "'";
    rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
        std::cout << "SQL error: " << sqlite3_errmsg(database) << std::endl;
    ...
    sql = "SELECT AVG(AAA) FROM Sensor009 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "'";
    rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
        std::cout << "SQL error: " << sqlite3_errmsg(database) << std::endl;
...
std::string SensorSoilMoisture::GetQualityControlBit()
{
    return "000000";
}

std::string SensorSoilMoisture::GetQualityControlBit_5Min()
{
    return "0";
}

uint8_t SensorSoilMoisture::GetNumberOfMeasureElement()
{
    return 6;
}

uint8_t SensorSoilMoisture::GetNumberOfMeasureElement_5Min()
{
    return 1;
}

uint8_t SensorSoilMoisture::GetNumberOfEquipmentStatus()
{
    return 1;
}

std::vector<std::string> SensorSoilMoisture::GetStatusInfo(std::string startTime, std::string endTime)
{
...
std::vector<std::string> SensorSoilMoisture::CheckMissingData(std::string startTime, std::string endTime, std::string filter)
{
    std::cout << "Start to check missing data for sensor Sensor009" << std::endl;
    auto database = GetDataStorageUnit()->GetDatabase();
    // Get data from database between startTime and endTime and FILTER equal to filter
    std::string sql = "SELECT * FROM Sensor009 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "' AND FILTER = '" + filter + "'";
    sqlite3_stmt* stmt;
    auto rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
...
void SensorSoilMoisture::Command_ReadData()
{
    std::string command = "";
}


std::vector<std::string> SensorSoilMoisture::GetSensorData(std::string startTime, std::string endTime, std::string filter)
{
...
std::vector<std::string> SensorSoilMoisture::CalculateData_5Min(std::string startTime, std::string endTime)
{
    auto database = GetDataStorageUnit()->GetDatabase();
