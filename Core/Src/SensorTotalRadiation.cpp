...
SensorTotalRadiation::SensorTotalRadiation(std::string zoneNumber, std::string serviceType,
...
: Sensor(zoneNumber, serviceType, equipmentIdentification, equipmentId, dataStorageUnit)
{
    ...
    std::string sql = "CREATE TABLE IF NOT EXISTS Sensor011 (" \
                      "TIME TEXT NOT NULL," \
                      "DATA TEXT NOT NULL," \
                      "AAA TEXT NOT NULL," \
                      "AAAI TEXT NOT NULL," \
...
SensorTotalRadiation::SensorTotalRadiation(std::string equipmentId, DataStorageUnit* dataStorageUnit)
...
void SensorTotalRadiation::StoreData(std::vector<std::string> data, std::string originalData)
{
    ...
    std::string sql = "INSERT INTO Sensor011 (TIME, DATA, AAA, AAAI, AAA5i, AAAl5i, FILTER) VALUES ('" + time + "', '" + originalData + "', '" + dataAAA + "', '" + dataAAAI + "', '" + dataAAA5i + "', '" + dataAAAl5i + "', " + filter + ")";
...
std::vector<std::string> SensorTotalRadiation::CalculateData(std::string startTime, std::string endTime)
{
    // Get data of current hour from database table Sensor011
    auto database = GetDataStorageUnit()->GetDatabase();
    ...
    std::string sql = "SELECT * FROM Sensor011 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "' ORDER BY TIME DESC LIMIT 1";
    sqlite3_stmt* stmt;
    ...
    // Get max AAA value
    sql = "SELECT MAX(AAA) FROM Sensor011 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "'";
    ...
    sql = "SELECT TIME FROM Sensor011 WHERE AAA = '" + AAAa + "'";
    rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
        std::cout << "SQL error: " << sqlite3_errmsg(database) << std::endl;
    ...
    // Get min AAAc value
    sql = "SELECT MIN(AAA) FROM Sensor011 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "'";
    rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
        std::cout << "SQL error: " << sqlite3_errmsg(database) << std::endl;
    ...
    sql = "SELECT TIME FROM Sensor011 WHERE AAA = '" + AAAc + "'";
    rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
        std::cout << "SQL error: " << sqlite3_errmsg(database) << std::endl;
    ...
    sql = "SELECT AVG(AAA) FROM Sensor011 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "'";
    rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
        std::cout << "SQL error: " << sqlite3_errmsg(database) << std::endl;
...
std::string SensorTotalRadiation::GetQualityControlBit()
{
    return "000000";
}

std::string SensorTotalRadiation::GetQualityControlBit_5Min()
{
    return "0";
}

uint8_t SensorTotalRadiation::GetNumberOfMeasureElement()
{
    return 6;
}

uint8_t SensorTotalRadiation::GetNumberOfMeasureElement_5Min()
{
    return 1;
}

uint8_t SensorTotalRadiation::GetNumberOfEquipmentStatus()
{
    return 1;
}

std::vector<std::string> SensorTotalRadiation::GetStatusInfo(std::string startTime, std::string endTime)
{
...
std::vector<std::string> SensorTotalRadiation::CheckMissingData(std::string startTime, std::string endTime, std::string filter)
{
    std::cout << "Start to check missing data for sensor Sensor011" << std::endl;
    auto database = GetDataStorageUnit()->GetDatabase();
    // Get data from database between startTime and endTime and FILTER equal to filter
    std::string sql = "SELECT * FROM Sensor011 WHERE TIME BETWEEN '" + startTime + "' AND '" + endTime + "' AND FILTER = '" + filter + "'";
    sqlite3_stmt* stmt;
    auto rc = sqlite3_prepare_v2(database, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
    {
...
void SensorTotalRadiation::Command_ReadData()
{
    std::string command = "";
}


std::vector<std::string> SensorTotalRadiation::GetSensorData(std::string startTime, std::string endTime, std::string filter)
{
...
std::vector<std::string> SensorTotalRadiation::CalculateData_5Min(std::string startTime, std::string endTime)
{
    auto database = GetDataStorageUnit()->GetDatabase();
